...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleProjectBlockImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, id: string) => {
    if (e.target.files && e.target.files[0] && editingProject) {
      try {
        ...
        updateProjectBlock(id, 'content', webpData);
        e.target.value = '';
      } catch (error) {
  ...
  const handleGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && editingProject) {
      try {
        ...
        const processedImages = await Promise.all(fileList.map(file => processImage(file, 2500)));
  ...
  const moveAtelierBlock = (column: 'left' | 'right', index: number, direction: 'up' | 'down') => {
    ...
    if (direction === 'up' && index > 0) {
      [list[index], list[index - 1]] = [list[index - 1], list[index-1]];
    } else if (direction === 'down' && index < list.length - 1) {
      [list[index], list[index + 1]] = [list[index + 1], list[index]];
    }
